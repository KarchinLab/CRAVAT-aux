'''
Remember that pfam_in_uniprot_conversion.py should be run also.
'''
...
array_convention_types_desired_in_order = ["Ensembl_TRS", "RefSeq_NT", "CCDS"]
...
#         -Makes a dictionary of the conversions from uniprot IDs to the IDs in other databases
#         -Make a dictionary of the uniprot IDs and the length of the amino acid sequence for that ID
...
def construct_full_uniprot_table(ucsc_table, \
    uniprot_to_other_databases_convertion_file, uniprot_fasta, \
    uniprot_full_table, dict_unique_uniProtId, cravat_transcripts_file):
    
    sent_by_raise = 0
    
    try:
        
        print 'Make EnsT vernum dict'
        enst_vernum_dict = make_enst_vernum_dict(cravat_transcripts_file)
    
        print 'Make uniprot to other db dict'
        succss_making_uniprot_refSeq_dict, \
            dict_uniprot_to_Other_database_Ids = \
            make_dict_uniprot_to_otherDatabases(
                uniprot_to_other_databases_convertion_file, \
                dict_unique_uniProtId, \
                enst_vernum_dict)
        if succss_making_uniprot_refSeq_dict == 0:
            sent_by_raise = 1
            raise Exception

        print 'Make uniprot aa seq length dict' 
        sucess_getting_AA_length, dict_uniprotID_AA_length = \
            make_dict_uniprotID_AA_seq_length(
                uniprot_fasta, \
                dict_uniprot_to_Other_database_Ids)
        ...
        print 'Make cravat transcript aa length dicts'        
        success_make_CRAVAT_transcript_dicts, \
            dict_CRAVAT_refSeq_AALength, dict_CRAVAT_ENSEMBL_AALength, \
            dict_CRAVAT_CCDS_AALength = \
            make_dicts_CRAVAT_transcripts_AA_length(
                cravat_transcripts_file, 1)
        if success_make_CRAVAT_transcript_dicts == 0:
            sent_by_raise = 1
            raise Exception  
              
        print 'Match cravat and uniprot'               
        success_getting_CRAVAT_transcript, dict_uniProtId_bestConvertionId = \
            get_CRAVAT_transcript_with_same_AA_length_to_uniProt_conversion_transcripts(
                dict_uniprotID_AA_length, dict_uniprot_to_Other_database_Ids, 
                dict_CRAVAT_refSeq_AALength, dict_CRAVAT_ENSEMBL_AALength, 
                dict_CRAVAT_CCDS_AALength)
        ...
        print 'Print write table'
        success_writing_full_table = write_full_uniprot_table(ucsc_table, \
            uniprot_full_table, dict_uniProtId_bestConvertionId, \
...
def make_dict_uniprot_to_otherDatabases(rf_path, dict_desired_uniProtId, enst_vernum_dict):
    success = 0
...
#             Sometimes in the file the uniprotId has unprot-#. We don't know why but we are combining them all together so there are no hyphens
            ...
            if uniProt_id in dict_desired_uniProtId:
                if convertion_type in convertion_types_desired:
                    if convertion_ID.startswith('ENST'):
                        if convertion_ID in enst_vernum_dict:
                            convertion_ID = enst_vernum_dict[convertion_ID]
                        else:
                            continue
...
def get_CRAVAT_transcript_with_same_AA_length_to_uniProt_conversion_transcripts(dict_uniprotID_AA_length, dict_uniprot_to_Other_database_Ids, dict_refSeq_AALength, dict_ENSEMBL_AALength, dict_CCDS_AALength):
    success = 0
...
#                 num_no_transfer += 1
        ...
        if exception_from_raise == 1:
            ...
            sys.stderr.write("An error occurred while in get_CRAVAT_transcript_with_same_AA_length_to_uniProt_conversion_transcripts()\n")
            sys.stderr.write(str(repr(e)))
            sys.stderr.write("\n")
            sys.stderr.write(traceback.format_exc())
            sys.stderr.write("\n")  
...
def find_other_database_ID_with_same_length_as_uniProt_ID(uniProtId, transcripts, dict_transcripts_length, uniprot_transcript_length, db_type):
    ...
    try:
        ...
        for transcript in transcripts:
            if transcript in dict_transcripts_length:
                cravat_full_transcript_id = dict_transcripts_length[transcript][0]
                other_transcript_length = dict_transcripts_length[transcript][1]
...
def write_full_uniprot_table(ucsc_table, uniprot_full_table, \
    dict_uniProtId_bestConvertionId, dict_uniprotID_AA_length):
    success = 0
    try:
...
#             get hugo
            uid = None
            if cravat_transcript[0:4] == "CCDS":
                uid = dict_CRAVAT_CCDS_AALength[cravat_transcript][1]
            elif cravat_transcript[0:4] == "ENST":
                uid = dict_CRAVAT_ENSEMBL_AALength[cravat_transcript][1]
            else:
                uid = dict_CRAVAT_refSeq_AALength[cravat_transcript][1]
...
def make_enst_vernum_dict(transc_table_path):
    out = {}
    with open(transc_table_path) as f:
        for l in f:
            toks = l.strip('\n').split()
            enst = toks[4]
            enst_nv = enst.split('.')[0]
            out[enst_nv] = enst
    return out
...
if __name__ == "__main__":
    ...
    dir = 'C:\\Users\\Kyle\\annot_liftover\\uniprot'

#     download these manually
    ucsc_table = os.path.join(dir,"spAnnot.bed")       #from UCSC
    uniprot_fasta = os.path.join(dir, "uniprot_sprot.fasta")     #from uniprot
    uniprot_to_other_databases_convertion_file = os.path.join(dir, "HUMAN_9606_idmapping.dat")       #from uniprot
    cravat_transcripts = os.path.join(dir, "CRAVAT_transcripts.txt")     #from Rick
    uid_hugo_cravat = os.path.join(dir, "uid_hugo.txt")                  #from Rick
    
#     these are created and used by this program
    distinct_uniprot_ids = os.path.join(dir, "distinct_human_uniProtIds.tsv")
    uniprot_full_table = os.path.join(dir, "Uniprot_and_Other_Database_Conversion.bed")
    database_table_input = os.path.join(dir, "database_table_input.tsv")
    ...
    #         This takes the table built by UCSC and adds columns for the corresponding refSeq Id and for the length of the amino acid
        ...
        construct_full_uniprot_table(ucsc_table, \
            uniprot_to_other_databases_convertion_file, uniprot_fasta, \
            uniprot_full_table, dict_unique_uniProtId, cravat_transcripts)
    elif int(input) == 2:
        print '2'
